# -*- coding: utf-8 -*-
"""
Created on Mon Apr  4 13:49:54 2022
...
def re_hf(f,z):
    ...
    sign_2=0
    zc=z
    m1=1.4*1.989*(10**30)*pow(10,1)
    m2=1.4*1.989*(10**30)*pow(10,1)
    tc=0
    S=0.0
    phic=0
    lambdag=1.1*10**21
...
def re_data(z,M,fmin,fmax):
    ...
    h1=[]
    h1_l=[]
    for i in np.arange(len(z),dtype=int):
        h.append(re_ht(f,z[i],M)[0][int(0.5*M)::1000])
        h_l.append(0)
        h1.append(re_ht1(f,z[i],M)[0][int(0.5*M)::1000])
        h1_l.append(1)
    return h,h_l,h1,h1_l


# z=np.linspace(3,4,200)
z=np.linspace(4,5,100)
fmin=0.2
fmax=0.8
M=2000000
h,h_l,h1,h1_l=re_data(z, M, fmin, fmax)

sig=h+h1
label=h_l+h1_l
n=len(h[0])
aa=np.array(sig).reshape(int(2*len(z)),n)
bb=np.array(label).reshape(int(2*len(z)),1)
data=np.hstack((aa,bb))
